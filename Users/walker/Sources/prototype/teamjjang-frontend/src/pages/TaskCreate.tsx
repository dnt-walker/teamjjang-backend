...
const TaskCreate = () => {
  ...
  useEffect(() => {
    const fetchProjectAndUsers = async () => {
      ...
      try {
        ...
        // 현재 사용자 정보 가져오기
        const currentUserResponse = await userApi.getUserByUsername(username);
  ...
  const handleSubmit = async (values: TaskFormValues) => {
    if (!projectId || !project || !currentUser) return;
    ...
    try {
      ...
      const taskData = {
        ...values,
        creator: currentUser,  // User 객체로 변경
